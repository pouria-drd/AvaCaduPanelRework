...
<template>
    <nav class="bg-white h-16 nav-shadow">
        <div class="flex justify-between 2md:justify-end items-center h-full">
            <img class="w-7 ml-4 2md:mr-4" src="../assets/images/acLogo.svg" alt="AvacaduLogo">

            <svg v-if="!isOpen" @click="ToggleMenu" class="mr-4 cursor-pointer 2md:hidden"
                xmlns="http://www.w3.org/2000/svg" width="22" height="20" viewBox="0 0 14 12" fill="none">
                <path d="M1 11H13M1 6H13M1 1H13" stroke="#80848F" stroke-width="2" stroke-linecap="round"
                    stroke-linejoin="round" />
            </svg>

            <svg v-else @click="ToggleMenu" class="mr-4 cursor-pointer 2md:hidden" xmlns="http://www.w3.org/2000/svg"
    ...
    <div v-if="isOpen && isMobile" class="w-[100vw] h-[100vh] bg-white opacity-20">

    </div>
    <Sidebar v-if="!isMobile || isOpen" />
</template>
